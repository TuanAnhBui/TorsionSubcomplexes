...
########################################
DeclareGlobalFunction("RationalCellularHomology");
InstallGlobalFunction(RationalCellularHomology,
function(C)
local lnth, Cells, nrCells,x,w,j,s,id, Mat,t,M,b,p,h,r,
      Mult, CLeftCosetElt, pos, hom, Elts, MatRank, y, BdMat,
      KillGAction, Reduction, LocalSparseMat;
...
#Print(1/0);
  for s in [1..C!.dimension(j-1)] do
    ...
    for t in b do
        ...
        Add(M[s],[AbsInt(t[1]),SignInt(t[1])*t[2]]);
    od;
  od;
#  Print("Representation matrix at degree ",j-1,"  ",M,"\n");
  # Mat[j]:=SmithNormalFormIntegerMat(TransposedMat(M));
  Mat[j]:=LocalSparseMat(M,C!.dimension(j-1),C!.dimension(j-2));
  # BdMat[j]:=M;
...
#     M[s]:=0;
# od;
Mat[lnth+2]:=LocalSparseMat([],0,0);
# BdMat[lnth+2]:=[M];
hom:=[];
for j in [1..lnth+1] do
    h:=[];
    r:=C!.dimension(j-1)-Rank(Mat[j])-Rank(Mat[j+1]);
  #   for s in [1..r] do
  # h[s]:=0;
  #   od;
  #   for s in [1..MatRank(Mat[j+1])] do
