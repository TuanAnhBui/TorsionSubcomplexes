...
run, bdr, FREE, cutOff, Boundaries, Coboundaries, ii, s,t, bool,BOOL,nn,
returnedData, vcd, admissibilityCheck;
...
## of the (n-1)-cell.     					##
##################################################################
local firstCell, secondCell, boundaryCell, firstStabilizer, secondStabilizer,
boundaryStabilizer, cellTriple, n, g, validated, remainingFusionCandidates,
controllingGroupID, S, S1, S2, t1,t2,g1,g2,x,CS,CS2,t;
remainingFusionCandidates := StructuralCopy(fusionCandidates);
  ...
  for cellTriple in fusionCandidates do
   ...
   and controllingGroupID =IdSmallGroup(ReduceModP(boundaryStabilizer,p))
    then
      S:=celldata[n][boundaryCell].TheMatrixStab;
      S1:=celldata[n+1][firstCell[2]].TheMatrixStab;
      S2:=celldata[n+1][secondCell[2]].TheMatrixStab;
      t1:=Position(celldata[n+1][firstCell[2]].BoundaryImage.ListIFace,boundaryCell);
      t2:=Position(celldata[n+1][secondCell[2]].BoundaryImage.ListIFace,boundaryCell);
      g1:=celldata[n+1][firstCell[2]].BoundaryImage.ListElt[t1];
      g2:=celldata[n+1][secondCell[2]].BoundaryImage.ListElt[t2];
      x:=g1*g2^-1;
      CS:=ConjugateGroup(S,g1^-1);
      CS2:=ConjugateGroup(S2,x^-1);
      t:=0;
      for g in CS do
        if Set(ConjugateGroup(S1,g))=Set(CS2) then
          t:=1;
          break;
        fi;
      od;
      if t=0 then RemoveSet(remainingFusionCandidates, cellTriple);
      fi;
...
# RepresentativeGAction:=function(H,K)
# local g;
#   for g in C!.elts do
#     if Set(ConjugateGroup(H,g))=Set(K) then
#       return g;
#     fi;
#   od;
#   return fail;
...
#	firstCell[2]," and ",secondCell[2],".\n");
    ...
    x:=BI1!.ListElt[t01]*BI2!.ListElt[t02]^-1;
    U:=ConjugateGroup(data[n+1][p2]!.TheMatrixStab,(x)^-1);
    V:=data[n+1][p1]!.TheMatrixStab;
    X:=ConjugateGroup(celldata[n][BI1!.ListIFace[t01]]!.TheMatrixStab,BI1!.ListElt[t01]^-1);
    ...
    Remove(data[n+1],p2);
    Remove(reducedTorsionCells[n+1],p2);
  od;
...
run:=run+Runtime();
Print("Get torsion subcomplex took ",run," milliseconds \n");
   torsionCells := sortedData!.torsionCells;
   numberOfTorsionCells := sortedData!.numberOfTorsionCells;
   celldata := sortedData!.celldata;
...
# Preparing torsion subcomplex information
# Record Boundaries and Coboundaries
run:=-Runtime();
Boundaries:=[];
for i in [2..N+1] do
  Boundaries[i]:=[];
  for ii in [1..Length(celldata[i])] do
    b:=Set(celldata[i][ii].BoundaryImage.ListIFace);
    Boundaries[i][ii]:=Concatenation([Length(b)],b);
  od;
od;
Boundaries[1]:=List(celldata[1],i->[1,0]);
Boundaries[N+2]:=[];
...
od;
Coboundaries[N+1]:=List(Boundaries[N+1],a->[0]);


##### Cut off cells by DVF ############
cutOff:=function(n)
local MBoundaries, MCoboundaries, UCoboundaries, UBoundaries, C,
  i,nplus1Cell,G,IdGred,H,IdHred,b,j,LCoboundaries,cob,pos,
  u,U,Gnew,IdGnew,Hnew,IdHnew,Free;

MCoboundaries:=Coboundaries[n+1];
MBoundaries:=Boundaries[n+1];
UCoboundaries:=Coboundaries[n+2];
UBoundaries:=Boundaries[n+2];
C:=Length(MCoboundaries);
if n>0 then
  LCoboundaries:=Coboundaries[n];
fi;
...
for i in FREE do
    if MCoboundaries[i][1]=1 then
      nplus1Cell:=MCoboundaries[i][2];
      if UCoboundaries[nplus1Cell][1]=0 then
        G := celldata[n+1][i]!.TheMatrixStab;;
        IdGred := IdSmallGroup(ReduceModP(G,p));
        H := celldata[n+2][nplus1Cell]!.TheMatrixStab;;
        IdHred := IdSmallGroup(ReduceModP(H,p));
        if IdGred=IdHred then
          Add(Free,i);
        fi;
    ...
    if UCoboundaries[nplus1Cell][1]=0 then
      ...
      if IdGred=IdHred then
        ...
        for j in StructuralCopy(b{[2..1+b[1]]}) do
         ...
         if t=2 then
           u:=MCoboundaries[j][2];
           if UCoboundaries[u]=0 then
             Gnew := celldata[n+1][j]!.TheMatrixStab;;
             IdGnew := IdSmallGroup(ReduceModP(Gnew,p));
             Hnew := celldata[n+2][u]!.TheMatrixStab;;
             IdHnew := IdSmallGroup(ReduceModP(Hnew,p));
             if IdGnew = IdHnew then
               Add(Free,j);
             fi;
           fi;
...
od;
Boundaries[n+2]:=UBoundaries;
Coboundaries[n+2]:=UCoboundaries;
Boundaries[n+1]:=MBoundaries;
Coboundaries[n+1]:=MCoboundaries;
if n>0 then
  Coboundaries[n]:=LCoboundaries;
fi;
...
Print("cut off follows DVF took ",run," milliseconds \n");
# Print(Boundaries,"\n");
# Boundaries:=List(Boundaries,w->Filtered(w,x->not x[1]=0));

reducedTorsionCells:=[];
for i in [1..N+1] do
  reducedTorsionCells[i]:=[];
  for ii in [1..Length(Boundaries[i])] do
    if not Boundaries[i][ii][1]=0 then
      Add(reducedTorsionCells[i],torsionCells[i][ii]);
    fi;
  od;
od;

 # Print(reducedTorsionCells);
#
# reducedTorsionCells :=StructuralCopy(torsionCells);
#
# run:=-Runtime();
...
#Print("reducedTorsionCells",reducedTorsionCells,"\n");
  ...
  run:=run+Runtime();
  Print("Extracting torsion cells information took  ",run," milliseconds \n");
#  originalData:=celldata;
...
#		vcd,"-dimensional ",groupName,"-cell complex ... \n");
  ...
  numberOfTorsionCells := [];
  for n in [0..vcd] do
   numberOfTorsionCells[n+1]:=Length(reducedTorsionCells[n+1]);
  od;
...
#Print("stabilizerCardinalities  ", stabilizerCardinalities,"\n");
   ...
   fusionCandidates := bifurcationFreeCells(fusionCandidates, reducedTorsionCells,
						 celldata);
#  Print("fusionCandidates:  ", fusionCandidates,"\n");
...
run:=run+Runtime();
Print("Merging process took ",run," milliseconds \n");
#####################################################################################
  ...
  return Objectify(HapTorsionSubcomplex,
            ...
            groupname:=groupname,
            torsionCells:=reducedTorsionCells,
            reducedTorsionCells:=reducedTorsionCells,
#            originalData:=data,
            celldata:= celldata ));
