...
exportFile := "./outputGamma0i_1inSL3ZiRFS_AUG22.txt";
AppendTo(exportFile,"\n A new session makes the following output : \n");
...
if Length(arg)=2 then M:=Minimum(N,arg[2]);
    ...
    local bdry, w, x, d, y, a, b, T, j, j1,j2, z, t, c, Flag, s, s1, ConnectToCenter,
          ...
          Boundaries,Stabilizers,Tcells, convexTcells,p, runTime;
  ...
  local x,z,j,w,TT, tau, boundaryOFtau, S, Ps, ps,countPrint, bool;
      ...
      w:=[];
      Ps:=[[1,1]];
if k>=4 then AppendTo(exportFile,"Number of orbits:  ",Length(Orb),"\n");fi;
j:=1;
              ...
              if OrbFlag[j]=0 then
                  ...
                  while j1<=Length(Orb[j]) do
                      if Flag[j][j1]=0 then
                          ...
                          if IsContractible(k-1,TT,bool) then
                            if k>=4 then AppendTo(exportFile,"Number of cells in T reaches ...",Length(TT),"\n");fi;
...
#    if e[1]=3 and k=4 then
#        AppendTo(exportFile,"Test for the cell s in S: \n");
#        for x in bdrye do

#            AppendTo(exportFile,"Number of cells in s and e(s):",NaiveEulerChar(e[1]-1,[x]),"\n");
#            w:=ConnectToCenter([e[1]-1,[[AbsInt(x[1]),x[2]]]]);
#            AppendTo(exportFile,NaiveEulerChar(e[1],[w]),"\n");
#            if not NaiveEulerChar(e[1],[w])[1]=1 then Print(1/0);fi;
...
#            AppendTo(exportFile,"Number of cells in S the boundary of T and e(T):",NaiveEulerChar(e[1]-1,bdrye));
            w:=AddCell(e[1]+1,stab,bdry1,e[2]);
#            AppendTo(exportFile,"Length of T ",Length(e[2]),NaiveEulerChar(e[1]+1,[w]),"\n");
            return w;
        else
...
############################ END CHECK##########################
	...
	## Start with the cells of boundary(sigma) = bdry[1] :
        ...
        AppendTo(exportFile,"Classifing cells into orbits took ",runTime," milliseconds \n");
...
if (k<=N) then
	...
	# and such that the naive Euler characteristic of this union remains 1.
        ...
        AppendTo(exportFile,"A fundamental domain found in ",runTime," milliseconds. \n");
        # AppendTo(exportFile,"\n");
        fi;
  #      AppendTo(exportFile,"  Naive Euler characteristic: ",NaiveEulerChar(k-1,T),"\n");
  #      AppendTo(exportFile,"  Homology of T:");
  #      AppendTo(exportFile,"  ",CellHomology(k-1,T), "\n");
  #      AppendTo(exportFile,"\n");
	...
	# Use Algorithm 3 to construct a fundamental domain F for sigma
        AppendTo(exportFile,"\n Constructing the envelope F of T and the barycenter \n");
        ...
        # Print(Tcells[k-1],"\n");
        AppendTo(exportFile,"\n F has been constructed in ",runTime," milliseconds. \n");

#        AppendTo(exportFile,"  Naive Euler characteristic of F: ",NaiveEulerChar(k,[d]),"\n");
#        AppendTo(exportFile,"  Homology of the ",k,"-cell F:");
#        AppendTo(exportFile,"  ",CellHomology(k,[d]), "\n");
        ...
        AppendTo(exportFile,"\n For the ",i,"th of the ",k,"-cell \n");
        AppendTo(exportFile,"\n Using Virtual Simplicial Subdivision ...\n");
        T :=List(Orb,i->i[1]);
     ###################################################################
        w:=[];
        BoundaryTemp:=[];
...
#     Print([nrCells,w],"\n");
    ...
    ################
    IsContractible:=function(k,L,bool)
      local Boundaries,Coboundaries, Cells, x, w, b, j, s, t, Y, crit, l, flag,
            count, d, S, nrCells, eulerChar, tau, boundaryOFtau,
            sCells, sBoundaries,sCoboundaries,Orientation,run,P;
...
#          Print("w=",w,"\n");
          ...
          od;
#          Print("Boundaries",Boundaries[j+1],"\n");
...
if k>=1 then
      ...
      for tau in L do
        # boundaryOFtau:=Mult(BoundaryRec[k-1]
           ...
           boundaryOFtau:=BoundaryMult(k,AbsInt(tau[1]),tau[2]);
        if  tau[1]<0 then
      ...
      while j>0 do
          w:=[];
          Orientation[j+1]:=[];
          for s in [1..Length(sCells[j+1])] do
              x:=sCells[j+1][s];
              # w[s]:=StructuralCopy(BoundaryRec[j][AbsInt(x[1])]);
              # w[s]:=Mult(w[s],j-1,x[2]);
              w[s]:=BoundaryMult(j,AbsInt(x[1]),x[2]);
              ...
              w[s]:=List(w[s],a->[AbsInt(a[1]),a[2]]);
              sCells[j]:=Union(sCells[j],w[s]);
          od;

          sBoundaries[j+1]:=[];
          for s in [1..Length(sCells[j+1])] do
                  b:=List(w[s],a->Position(sCells[j],a));
                  sBoundaries[j+1][s]:=Concatenation([Length(b)],b);
          od;
#          Print("Boundaries",Boundaries[j+1],"\n");

          j:=j-1;
      od;
      nrCells:=List([1..k],i->Length(sCells[i]));
      # Naive Euler Char of L=Union of T and \tau
      eulerChar:=Sum(List([1..k],i->(-1)^(i+1)*nrCells[i]));
      if not eulerChar=1+(-1)^(k-1) then return false;fi;
fi;
      ################################################################
...
#      Print("\n",k,"  critical cells ",crit,"\n");
      if not (Length(crit)=1 and crit[1][1]=0) then return false;
      fi;
if k>=3 and bool=true then
  AppendTo(exportFile,"Start computing fundamental group of S ...\n");
  run:=-Runtime();
  sBoundaries[1]:=List(sCells[1],a->[1,0]); # denote the boundary of 0-cell
  Orientation[1]:=List(sCells[1],a->[1]);
  sBoundaries[k+1]:=[];
  ######BOUNDARIES END########
  ######COBOUNDARIES BEGIN####
  sCoboundaries:=[];; #Coboundaries[n+1] contains the info on n-cells.
  for j in [0..k-1] do
#        t:=j+3;
    sCoboundaries[j+1]:=List(sBoundaries[j+1],i->[0]);
    for s in [1..Length(sBoundaries[j+2])] do
      b:=sBoundaries[j+2][s];
      t:=Length(b);
      for i in b{[2..t]} do
        sCoboundaries[j+1][i][1]:=sCoboundaries[j+1][i][1]+1;
        Add(sCoboundaries[j+1][i],s);
      od;
    od;

  od;
  sCoboundaries[k]:=List(sBoundaries[k],a->[0]);
  #####COBOUNDARIES END######
  Y:=ListsOfCellsToRegularCWComplex(k-1,sBoundaries,sCoboundaries,Orientation);
  P:=FundamentalGroupOfRegularCWComplex(Y);
  run:=run+Runtime();

  AppendTo(exportFile,"Computing Fundamental group took ",run,"milliseconds \n");
#      Print("\n",k,"  critical cells ",crit,"\n");
  if (IsTrivial(P)) then return true;
    ...
    IsSimplyConnected:=function(k,L,bool)
      local Boundaries,Coboundaries, Cells, x, w, b, j, s, t, Y, crit, l, flag,
            count, d, S, nrCells, eulerChar, P, Orientation, run;
...
#    Print("    The naive euler characteristic: ",w,"\n");
  ...
  od;
    ...
    for j in [1..k+1] do
        ...
        od;
        Add(w,h);
  #      AppendTo(exportFile,"    The ",j-1,"th homology group",h,"\n");
    od;
...
local W, StabRec, i, j, N, M, x, bdry, s1, s2, p, k, w, t,
      ...
      Stabilizer, Action, IsRigidCell, ReplaceCell, SubdividingCell,
      Orbit, C, NaiveEulerChar, CellHomology, exportFile, IsContractible,
      CosetRec, IsSimplyConnected, BoundaryMult, BoundaryMultRec, jj, jjj,switch;
...
## Initialisation of output file
exportFile := "./output_HybridSub.txt";
AppendTo(exportFile,"\n A new session makes the following output : \n");

switch:=N;
if Length(arg)=2 then switch:=arg[2];
fi;
...
############################ END CHECK##########################
	...
	## Start with the cells of boundary(sigma) = bdry[1] :
     ...
     ##################################################################
        ...
        OrbFlag[1]:=1;
if (k<switch) then
    AppendTo(exportFile,"Using Rigid Facet Subdivision ... \n");
        T :=[Orb[1][1]];
        # In each orbit {g_{jt} sigma_j with j >= 2,
	# choose one cell such that its union with the cells in T is connected,
	# and such that the naive Euler characteristic of this union remains 1.
...
#        AppendTo(exportFile,"  ",CellHomology(k,[d]), "\n");
  ...
  # Use Algorithm 3 to construct a fundamental domain F for sigma
        ...
        od;
        runTime:=runTime+Runtime();
        AppendTo(exportFile,"FINISHED! Virtually connect to Center took ",runTime," milliseconds \n");
        return w;
      fi;
    end;
    ##################################################################
...
## Initialisation of output file
exportFile := "./outputPSL4Z_VSS_AUG22.txt";
AppendTo(exportFile,"\n A new session makes the following output : \n");
